...
function SelectDateFormEnvoiLingala(props)
{
  ...
  const testValidation = yup.object().shape({
    ...
    const submitVol =(values)=>
    {
     ...
     const date = { "dateInfo": day.concat("",mois,year)}
      ...
      console.log(date)
      setModalShow2(true)
        fetch('https://kobobsapi.herokuapp.com/api/getDailyRapportInfo/', {
            method:'POST',
          ...
          .then(
            res => {   
               ...
               navigate('/daily_rapport_envoi_lingala')
            }
          )
          .catch( (error) =>
            {
...
<Row className='mt-3'>
        <Col xs={12} className="text-start text-light">
            <p><Link to='/menu_rapport_envoi_lingala' style={{textDecoration:"none"}}><b className='text-light'>Rapport yaba envois &gt;&gt; </b></Link> <Link to='/select_date_form_envoi_lingala' style={{textDecoration:"none"}}><b className='text-warning'>Formulaire ya rapport journalier</b></Link></p>
        </Col>
    </Row>
...
export default SelectDateFormEnvoiLingala;