...
function SelectDateFormRecetteEnglish(props)
{
  ...
  const testValidation = yup.object().shape({
    ...
    const [message,setMessage] = useState("select date for daily incomes rapport")
        ...
        fetch('https://kobobsapi.herokuapp.com/api/getDailyRapportRecetteInfo/', {
          ...
          .then(
            res => {   
               ...
               navigate('/daily_rapport_recette_english')
            }
          )
          .catch( (error) =>
            {
...
export default SelectDateFormRecetteEnglish;