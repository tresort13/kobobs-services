...
function SelectDateFormRecetteFrench(props)
{
  ...
  const testValidation = yup.object().shape({
    ...
    const [message,setMessage] = useState("Selectionner la date pour le rapport les récettes journalières")
        ...
        fetch('https://kobobsapi.herokuapp.com/api/getDailyRapportRecetteInfo/', {
          ...
          .then(
            res => {   
               ...
               navigate('/daily_rapport_recette_french')
            }
          )
          .catch( (error) =>
            {
...
export default SelectDateFormRecetteFrench;