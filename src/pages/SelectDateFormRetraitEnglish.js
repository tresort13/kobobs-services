...
function SelectDateFormRetraitEnglish(props)
{
  ...
  const testValidation = yup.object().shape({
    ...
    const [message,setMessage] = useState("Select date for daily withdrawal rapport")
        ...
        fetch('https://kobobsapi.herokuapp.com/api/getDailyRapportInfoRetrait/', {
          ...
          .then(
            res => {   
               ...
               navigate('/daily_rapport_retrait_english')
            }
          )
          .catch( (error) =>
            {
...
export default SelectDateFormRetraitEnglish;