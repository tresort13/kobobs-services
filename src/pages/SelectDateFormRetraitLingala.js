...
function SelectDateFormRetraitLingala(props)
{
  ...
  const testValidation = yup.object().shape({
    ...
    const [message,setMessage] = useState("Pona date ya rapport ya rétrait ya mokolo")
    ...
    const submitVol =(values)=>
    {
     ...
     const date = { "dateInfo": day.concat("",mois,year)}
      ...
      console.log(date)
      setModalShow2(true)
        fetch('https://kobobsapi.herokuapp.com/api/getDailyRapportInfoRetrait/', {
            method:'POST',
          ...
          .then(
            res => {   
               ...
               navigate('/daily_rapport_retrait_lingala')
            }
          )
          .catch( (error) =>
            {
...
export default SelectDateFormRetraitLingala;