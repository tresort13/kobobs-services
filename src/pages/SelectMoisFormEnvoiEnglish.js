...
function SelectMoisFormEnvoiEnglish(props)
{
  ...
  const testValidation = yup.object().shape({
    ...
    const [message,setMessage] = useState("Select month for daily sending rapport")
      ...
      setModalShow2(true)
        fetch('https://kobobsapi.herokuapp.com/api/getMonthlyRapportInfo/', {
          ...
          .then(
            ...
            navigate('/monthly_rapport_envoi_english')
            }
          )
          .catch( (error) =>
            {
...
export default SelectMoisFormEnvoiEnglish;