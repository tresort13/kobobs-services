...
function SelectMoisFormRetraitEnglish(props)
{
  ...
  const testValidation = yup.object().shape({
    ...
    const [message,setMessage] = useState("Select month for monthly withdrawal rapport")
      ...
      setModalShow2(true)
        fetch('https://kobobsapi.herokuapp.com/api/getMonthlyRapportInfoRetrait/', {
          ...
          .then(
            ...
            navigate('/monthly_rapport_retrait_english')
            }
          )
          .catch( (error) =>
            {
...
export default SelectMoisFormRetraitEnglish;