...
function SelectMoisFormRetraitFrench(props)
{
  ...
  const testValidation = yup.object().shape({
    ...
    const [message,setMessage] = useState("Selectionner le mois pour le rapport de rétrait mensuel")
      ...
      setModalShow2(true)
        fetch('https://kobobsapi.herokuapp.com/api/getMonthlyRapportInfoRetrait/', {
          ...
          .then(
            ...
            navigate('/monthly_rapport_retrait_french')
            }
          )
          .catch( (error) =>
            {
...
export default SelectMoisFormRetraitFrench;