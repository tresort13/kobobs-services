...
function SelectMoisFormRetraitLingala(props)
{
  ...
  const testValidation = yup.object().shape({
    ...
    const [message,setMessage] = useState("Pona mois ya rapport ya rétrait ya sanza")
    ...
    const submitVol =(values)=>
    {
     ...
     const date = { "moisInfo": mois.concat("/",year)}
      ...
      console.log(date)
      setModalShow2(true)
        fetch('https://kobobsapi.herokuapp.com/api/getMonthlyRapportInfoRetrait/', {
            method:'POST',
          ...
          .then(
            ...
            navigate('/monthly_rapport_retrait_lingala')
            }
          )
          .catch( (error) =>
            {
...
<Row className='mt-3'>
        <Col xs={12} className="text-start text-light">
            <p><Link to='/menu_rapport_retrait_lingala' style={{textDecoration:"none",fontSize:20}}><b className='couleur2'>&#8592; <u>Zonga</u>  </b></Link> </p>
        </Col>
    </Row>
...
export default SelectMoisFormRetraitLingala;