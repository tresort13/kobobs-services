...
function SelectYearFormRecetteFrench(props)
{
  ...
  const testValidation = yup.object().shape({
    ...
    const [message,setMessage] = useState("Entrer l'année pour le rapport des recettes annuelle")
   ...
   //  const day = values_replace.slice(8,10)
     const date = { "yearInfo": values.dateInfo}
      ...
      console.log(date)
      setModalShow2(true)
        fetch('https://kobobsapi.herokuapp.com/api/getYearlyRapportRecetteInfo/', {
            method:'POST',
          ...
          .then(
            res => {   
               ...
               navigate('/yearly_rapport_recette_french')
            }
          )
          .catch( (error) =>
            {
...
<Row className='mt-3'>
        <Col xs={12} className="text-start text-light">
            <p><Link to='/menu_rapport_recette_french' style={{textDecoration:"none",fontSize:20}}><b className='couleur2'>&#8592; <u>Retourner</u>  </b></Link> </p>
        </Col>
    </Row>
...
export default SelectYearFormRecetteFrench;