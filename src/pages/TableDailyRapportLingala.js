...
function TableDailyRapportLingala(props)
{
...
const export_excel = ()=>{
  ...
  if(rapportLocation === "Rapport Angola et RD Congo")
  {
    dailyRecettes = props.detailEnvoieTotalTableau.map((value)=>{
     ...
     title = "Angola na RD Congo "+props.dateInfo.replaceAll('/','_')+""
    console.log(title)
    dailyRecettes.push({
      date : "TOTAL",
      withdrawal_code : "",
 ...
 /* fix headers */
 utils.sheet_add_aoa(worksheet, [["Dati","Code ya rétrait", "Kombo Motindi","Numéro ya Motindi","Kombo Mozui","Mboka Ya Mozui","Montant ya Mozui($)"]], { origin: "A1" });
...
<Row className='justify-content-center '>
        ...
        <div>
        <h4 ><u><b><i className='couleur2'>{props.titleLingala} {props.dateInfo}</i></b></u></h4>
        </div>
        <div>
    ...
    <Form.Select aria-label="Default select example" onChange={(e)=>changeRapportLocation(e)}>
      ...
      <option value="Rapport RD Congo"><b>Rapport ya RD Congo</b></option>
      <option value="Rapport Angola"><b>Rapport ya Angola</b></option>
    </Form.Select>
    </Form>
    </div> : <di></di>}
...
export default TableDailyRapportLingala;